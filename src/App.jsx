...
function AppShell({ currentUser, onLogout }) {
  ...
  useEffect(() => {
    const userId = supabase ? supabase.auth.getUser().then(({ data }) => data?.user?.id) : null;
    if (userId) {
      userId.then(id => {
        if (id) registerPush(id, currentUser.gezin_code);
      });
    }
  ...
  useEffect(() => {
    const unread = messages.filter(m => m.van !== currentUser.naam).length;
